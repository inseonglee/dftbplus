...
contains
      ...
      if (tSaveMem) then
        ...
        if (isHalf) then
          ...
          call HxcKernelHalf_(getDenseAO, overSqr, GammaAO, SpinAO, OnsiteAO, LrGammaAO, &
              & LrOnsiteAO, isOnsite, isHybridXc, isRS_OnsCorr, isHalf, HxcHalfS, HxcHalfD)
  ...
  subroutine getZmat(env, denseDesc, neighbourList, nNeighbourSK, &
      ...
      & HxcHalfD, HxcSpS, HxcSpD, overSqr, over, GammaAO, SpinAO, OnsiteAO, &
      & LrGammaAO, LrOnsiteAO, orderRmatL, getDenseAO, Lpaired, Glevel, &
      & tSaveMem, isOnsite, isHybridXc, isRS_OnsCorr, isHalf, ZmatL)
    ...
    if (Glevel == 1 .or. Glevel == 2) then
      ...
      if (tSaveMem) then
        ...
        call getZmatNoHxc_(env, denseDesc, neighbourList, nNeighbourSK, &
            ...
            & OnsiteAO, LrGammaAO, LrOnsiteAO, overSqr, RmatL, orderRmatL, &
            & Lpaired, isOnsite, isHybridXc, isRS_OnsCorr, isHalf, ZmatL)
...
!$OMP END PARALLEL DO
  ...
  subroutine RTshift(env, sccCalc, denseDesc, neighbourList, nNeighbourSK, &
    ...
    if (isHybridXc .or. isRS_OnsCorr) then
      ...
      call getSPmatrices(deltaRhoSqrL, overSqr, isHybridXc, SP, SPS)
    end if
    ...
    contains
      ...
      subroutine getSPmatrices(deltaRhoSqrL, overSqr, isHybridXc, SP, SPS)
        ...
        real(dp), allocatable, intent(inout) :: SPS(:,:,:)
...
!$OMP END PARALLEL DO
  ...
  subroutine getZmatNoHxc_(env, denseDesc, neighbourList, nNeighbourSK, &
    ...
    ! common variables
    real(dp) :: tmp22!, tmp11
    integer :: mu, nu, tau, gam, kap, nOrb, iL, Lmax, LmaxR
    ...
    ! calculate ZmatL for scc, spin, onsite terms

!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(mu,nu,tau,gam, &
!$OMP& tmpG1,tmpG2,tmpG3,tmpG4,tmpS1,tmpS2,tmpS3,tmpS4, &
!$OMP& tmpO1s,tmpO2s,tmpO3s,tmpO4s,tmpO1d,tmpO2d,tmpO3d,tmpO4d, &
!$OMP& tmpvalue1s,tmpvalue1d,tmpHxcS,tmpHxcD,tmpZM) SCHEDULE(RUNTIME)
    do ii = 1, sparseSize
      ...
      if (tau <= gam .and. abs(overSqr(tau,gam)) >= epsilon(1.0_dp)) then
        ...
        ! calculate the H-XC kernel for scc, spin, onsite terms
        tmpHxcS(:) = 0.0_dp
        tmpHxcD(:) = 0.0_dp
        do jj = 1, sparseSize
          ...
          if (mu <= nu .and. abs(overSqr(mu,nu)) >= epsilon(1.0_dp)) then
            ...
            if (isOnsite) then
              ...
              tmpHxcS(jj) = tmpHxcS(jj) - tmpvalue1s
              tmpHxcD(jj) = tmpHxcD(jj) - tmpvalue1d
...
!$OMP END PARALLEL DO
    ...
    ! calculate the ZmatL for LC, onsite terms
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(mu,nu,tau,gam, &
!$OMP& tmp22,tmpHxcS,tmpHxcD,tmpvalue1s,tmpvalue2s,tmpvalue1d, &
!$OMP& tmpL1,tmpL2,tmpL3,tmpL4,tmpO1s,tmpO2s,tmpO3s,tmpO4s, &
!$OMP& tmpO1d,tmpO2d,tmpO3d,tmpO4d,tmpZM) SCHEDULE(RUNTIME)
      do ii = 1, nOrbHalf
        ...
        ! calculate the H-XC kernel for LC, onsite terms
        tmpHxcS(:) = 0.0_dp
        if (isOnsite) then
          tmpHxcD(:) = 0.0_dp
        end if
        do jj = 1, nOrbHalf
          ...
          if (isHybridXc) then
            ...
            tmpHxcS(jj) = tmpHxcS(jj) - tmpvalue1s - tmpvalue2s
          ...
          if (isOnsite) then
            ...
            tmpHxcS(jj) = tmpHxcS(jj) + 0.25_dp * tmpvalue1s
            tmpHxcD(jj) = tmpHxcD(jj) + 0.25_dp * tmpvalue1d
          ...
          if (isRS_OnsCorr) then
            ...
            tmpHxcS(jj) = tmpHxcS(jj) - 0.125_dp * tmpvalue1s
            ...
            tmpHxcS(jj) = tmpHxcS(jj) + 0.5_dp * tmpvalue1s
            ...
            tmpHxcS(jj) = tmpHxcS(jj) - 0.125_dp * tmpvalue2s
            ...
            tmpHxcS(jj) = tmpHxcS(jj) + 0.5_dp * tmpvalue2s
...
!$OMP END PARALLEL DO
  ...
  subroutine getOnsiteTerms_(Sderiv, rhoSqrL, overSqr, qBlockL, q0, OnsiteAO, &
    ...
    real(dp), allocatable :: tmpOnsite(:,:,:,:)          ! mOrb, mOrb, nSpin, 2

    real(dp), allocatable :: sumRI(:)                    ! mOrb
...
!$OMP END PARALLEL
  ...
  #:endif
    ...
    allocate(tmpOnsite(mOrb,mOrb,nSpin,2))
    ...
    ! compute a matrix from qBlockL multiplied by full-range onsite constants
...
!$OMP& sumRI,shiftPS,iAtom3,iAtom4) REDUCTION(+:deriv1,deriv2) SCHEDULE(RUNTIME)
    ...
    #:endif
      ...
      if (iAtom1 /= iAtom2) then
        ...
        tmpOnsite(:,:,:,:) = 0.0_dp
        do iSpin = 1, nSpin
          tmpOnsite(1:iAtTau2-iAtTau1+1,1:iAtTau2-iAtTau1+1,iSpin,1) &
              & = OnsiteAO(iAtTau1:iAtTau2,iAtTau1:iAtTau2,iSpin,1)
          tmpOnsite(1:iAtGam2-iAtGam1+1,1:iAtGam2-iAtGam1+1,iSpin,2) &
              & = OnsiteAO(iAtGam1:iAtGam2,iAtGam1:iAtGam2,iSpin,1)
        end do
        ...
        ! 1st : overlap derivative
        do iL = 1, Lmax
          shiftIM(:,:,:) = 0.0_dp
          do ii = 1, 3
            shiftIM(:,:,ii) = matmul( LmulP(:,:,iAtom1,iL),tmpSderiv(:,:,ii) ) &
                & + matmul( tmpSderiv(:,:,ii),LmulP(:,:,iAtom2,iL) )
          end do
        ...
        do iL = 1, Lmax
          ...
          do ii = 1, 3
            ...
            do iSpin = 1, nSpin
              ! P * dS^T + dS * P^T term
              shiftMM(:,:) = 0.0_dp
              shiftMM(:,:) = matmul( tmpRho(:,:,iSpin,iL),transpose(tmpSderiv(:,:,ii)) ) &
                  & + matmul( tmpSderiv(:,:,ii),transpose(tmpRho(:,:,iSpin,iL)) )
              shiftOM(:,:) = shiftOM(:,:) + tmpOnsite(:,:,iSpin,1) * shiftMM(:,:)
            end do
            call gemm(shiftIM2(:,:,ii), shiftOM, tmpOver2, alpha=0.5_dp)
          end do
          ...
          do ii = 1, 3
            ...
            do iSpin = 1, nSpin
              ...
              shiftOM(:,:) = shiftOM(:,:) + tmpOnsite(:,:,iSpin,2) * shiftMM(:,:)
            end do
            call gemm(shiftIM1(:,:,ii), tmpOver1, shiftOM, alpha=0.5_dp)
          end do
          Tterm1(:,:,:,iL) = Tterm1(:,:,:,iL) + shiftIM1

        end do

        ! 3rd : overlap derivative related to RI loss correction
        do iL = 1, Lmax
          shiftIM(:,:,:) = 0.0_dp
          do ii = 1, 3
        ...
        ! 4th : overlap derivative in dual density matrix related to RI loss correction
        do iL = 1, Lmax
          ...
          do ii = 1, 3
            ...
            do iSpin = 1, nSpin
              ...
              do mu = iAtTau1, iAtTau2
                sumRI(mu-iAtTau1+1) = sumRI(mu-iAtTau1+1) + &
                    & sum(OnsiteAO(iAtTau1:iAtTau2,mu,iSpin,2)*shiftPS(:),dim=1)
              end do
            end do
            do mu = 1, iAtTau2 - iAtTau1 + 1
              shiftIM2(mu,:,ii) = - 2.0_dp * sumRI(mu) * tmpOver2(mu,:)
            end do
          end do
          Tterm2(:,:,:,iL) = Tterm2(:,:,:,iL) + shiftIM2
          ...
          do ii = 1, 3
            ...
            do iSpin = 1, nSpin
              ...
              do nu = iAtGam1, iAtGam2
                sumRI(nu-iAtGam1+1) = sumRI(nu-iAtGam1+1) + &
                    & sum(OnsiteAO(iAtGam1:iAtGam2,nu,iSpin,2)*shiftPS(:),dim=1)
              end do
            end do
            do nu = 1, iAtGam2 - iAtGam1 + 1
              shiftIM1(:,nu,ii) = - 2.0_dp * sumRI(nu) * tmpOver1(:,nu)
            end do
          end do
          Tterm1(:,:,:,iL) = Tterm1(:,:,:,iL) + shiftIM1
...
!$OMP END PARALLEL DO
    ...
    contains
      ...
      !> compute a matrix from qBlockL multiplied by full-range onsite constants
      subroutine getLmulP(deltaQBlockL, OnsiteAO, iSquare, LmulP)
        ...
        do iL = 1, Lmax
          do iSpin = 1, nSpin
            do iAtom = 1, nAtom
              ...
              do ii = 1, iAt2 - iAt1 + 1
                do jj = 1, iAt2 - iAt1 + 1
                  LmulP(ii,jj,iAtom,iL) = LmulP(ii,jj,iAtom,iL) + &
                      & deltaQBlockL(ii,jj,iAtom,iSpin,iL) * OnsiteAO(iAt1+ii-1,iAt1+jj-1,iSpin,1)
                end do
              end do
            end do
          end do
